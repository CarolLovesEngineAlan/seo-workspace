      ...
      <div className="mx-auto max-w-[1480px] px-4 py-6 md:px-6">
        <header className={`${headerPanelClass} mb-6 px-[22px] py-5`}>
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              <div className="text-[15px] font-semibold">Prompt Library</div>
              <p className="mt-1 text-[12px] text-[#5e6860]">
                管理所有 SEO 落地页和博客文案的 Prompt 模板，按页面类型和步骤分类。
              </p>
            </div>
        ...
        {loadError ? (
          ...
          <PromptsTable prompts={prompts} canEdit={access.permissions.canWrite} />
        )}
      </div>
    </div>
  );
